...
5. The Cloudflare article appeared in `#chaser-agent-build`, while the active paper scout delivers to the auto-research channel; this means useful build-channel links need an intake path instead of relying on ad hoc Discord memory.
6. Existing visual completion evaluator work is implemented and green, but it is uncommitted and should be staged/committed after review.

## New practice added in this pass

The repo now explicitly recognizes **industry practice blogs** as a research intake class, starting with Cloudflare:
...
- repo releases: GitHub releases and changelogs;
- internal ChaseOS proof artifacts.

### 2. Monetized-resource gate

Before Chaser Agent accesses a source/tool that might charge money, it should emit a review packet:
...
- usage-cost ledger;
- approval consumption only through ChaseOS Gate/control plane;
- no auto-subscribe / no auto-pay / no wallet / no stablecoin settlement without explicit future authority.
...
### Slice A — Commit the visual completion evaluator seed

Status: implemented, tests green, not committed.

Required before commit:

- inspect generated `src/chaser_agent.egg-info/` and `uv.lock` to decide whether to keep or remove;
- run full test suite;
- stage intentional files only;
- commit with message such as `feat: add visual completion evaluator seed`.

### Slice B — Blog/industry intake contract

Status: started in this pass by adding Cloudflare to `research_intake/sources.yaml` and a config regression test.

Next TDD target:

- add deterministic source-card style artifact for industry-practice URLs;
...
- no login/session/account profile;
- read-only fetch/render/screenshot only;
- detect article metadata and monetization/payment signals;
...
- how receipts would be stored if ever authorized.
